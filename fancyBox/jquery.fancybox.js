...
//                        afterShow: function(){ jQgis('.fancybox-image').addpowerzoom();}, // After opening
		...
		_start: function (index) {
			...
			$.each(['href', 'title', 'content', 'type', 'alt'], function(i,v) {
	...
	F.helpers.title = {
        ...
        F.helpers.alt = {
            beforeShow: function (opts) {
                ...
                var alt, text = F.current.alt;
                
                if (text) {
                    alt = $('<div class="fancybox-alt fancybox-alt-' + opts.type + '-wrap">' + text + '</div>').appendTo('body');

                if (opts.type === 'float') {
                    //This helps for some browsers
                    alt.width(alt.width());
                    
                    alt.wrapInner('<span class="child"></span>');
                    
                    //Increase bottom margin so this alt will also fit into viewport
                    F.current.margin[2] += Math.abs(parseInt(alt.css('margin-bottom'), 10));
                }

                alt.appendTo(opts.type === 'over' ? F.inner : (opts.type === 'outside' ? F.wrap : F.outer));
                                }
            }
        };
